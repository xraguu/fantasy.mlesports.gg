...
export default function HomePage() {
  ...
  const [managerStatsTab, setManagerStatsTab] = useState<"global" | "league">(
    "global",
  );
    ...
    const fetchLeagueStandings = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/leagues/${standingsLeagueId}/standings`,
        );
        if (response.ok) {
          const data = await response.json();
                      ...
                      >
                        <td
                          style={{
                            ...
                            fontSize: "1rem",
                            color:
                              player.rank <= 3 ? "var(--accent)" : "inherit",
                          }}
                        >
                          {player.rank}
                        </td>
                ...
                <tbody>
                  {!standingsLeagueId ? (
                    ...
                    leagueStandings.map((team) => {
                      ...
                      const winRate =
                        totalGames > 0 ? (team.wins / totalGames) * 100 : 0;
                      return (
                        <tr
                          ...
                          style={{
                            borderBottom: "1px solid rgba(255,255,255,0.05)",
                            backgroundColor: team.isYou
                              ? "rgba(242, 182, 50, 0.08)"
                              : "transparent",
                            borderLeft: team.isYou
                              ? "3px solid var(--accent)"
                              : "3px solid transparent",
                            cursor: "pointer",
                          }}
                        ...
                        >
                          <td
                            style={{
                              ...
                              fontSize: "1rem",
                              color:
                                team.rank <= 3 ? "var(--accent)" : "inherit",
                            }}
                          >
                            {team.rank}
                          </td>
            ...
            >
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  backgroundColor:
                    gameMode === "all" ? "var(--accent)" : "transparent",
